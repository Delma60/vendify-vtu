...
function statusColor(s: RecentTransaction["status"]) {
  if (s === "success")
    return { text: B.green, bg: B.greenLight, label: "Success" };
    ...
    <Card className="p-5">
      <div className="mb-4 flex items-start justify-between">
        ...
        <span
          ...
          style={{
            background: up ? B.greenLight : B.redLight,
      ...
      <p className="mt-0.5 text-sm font-medium" style={{ color: B.textMuted }}>
        {card.label}
      </p>
...
function TxnStatusIcon({ status }: { status: RecentTransaction["status"] }) {
  if (status === "success")
    return <CheckCircle2 size={14} style={{ color: B.green }} />;
  if (status === "failed")
    return <XCircle size={14} style={{ color: B.red }} />;
...
export default function AdminDashboard() {
  ...
  async function load() {
    ...
    try {
      ...
      const mappedRecent: RecentTransaction[] = (
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs" style={{ color: "#B45309" }}>
                  Ogdams is experiencing high latency. Traffic has been rerouted
                  to Bilal.
                </p>
              </div>
            </div>
