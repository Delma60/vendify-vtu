...
// NOTE: /dashboard must be exempt so the post-PIN-set redirect doesn't loop
const PIN_EXEMPT = [
  '/set-pin',
  '/api/auth/set-pin',
  '/api/auth/logout',
  '/dashboard',   // ← was missing; caused redirect loop after PIN set
  '/profile',
  '/settings',
  '/api/auth',    // all auth endpoints are safe
];

export async function pinCheckMiddleware(request: NextRequest): Promise<NextResponse | null> {
  const { pathname } = request.nextUrl;

  // Skip exempt paths first (before anything else)
  if (PIN_EXEMPT.some((p) => pathname === p || pathname.startsWith(p + '/'))) return null;
  ...
  if (!token) return null; // auth middleware handles unauthenticated users
